...
for k,v in params.items():
    ...
    if 'b' in k:
         for i in range(v.shape[0]):
            ...
            params_minus[k] -= eps * delta
            h1_minus = forward(xb, params_minus, 'layer1')
            probs_minus = forward(h1_minus, params_minus, 'output', softmax)
            loss_minus, _ = compute_loss_and_acc(yb, probs_minus)

            params_plus = copy.deepcopy(params)
            params_plus[k] += eps * delta
            h1_plus = forward(xb, params_plus, 'layer1')
            probs_plus = forward(h1_plus, params_plus, 'output', softmax)
            loss_plus, _ = compute_loss_and_acc(yb, probs_plus)
